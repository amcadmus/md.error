...
% \newcommand{\recheck}[1]{{\textbf{#1}}}
% \newcommand{\redc}[1]{{\textbf{#1}}}
...
% estimate~\ref{eqn:c-error-water}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{correlation.eps}
...
% \ref{eqn:tmp10} and \ref{eqn:tmp18}:
% \begin{align}  
...
% \ref{eqn:tmp6} will result in a computational cost of $\mathcal
...
% the last term in \ref{eqn:ik.error.force.1},
...
%   \rangle.
% \end{align}
% In \ref{eqn:error-total}, the estimates of errors
% $\langle\,\vert\Delta\v F_{\textrm{dir}}(\v r)\vert^2\rangle $,
...
% In practice, the correlation~\ref{eqn:error-total-corr}
...
% \ref{eqn:tmp10}, \ref{eqn:tmp18} and \ref{eqn:ana.meanf}, all
...
\recheck{
  ...
  % the leading order
  % approximation~\ref{eqn:leading-order-1} and \ref{eqn:leading-order-2}.  
  ...
  when $R_g$
  % includes all major structures in the RDFs, i.e.
...
\ref{eqn:leading-order-1} and \ref{eqn:leading-order-2}
may play a role
% in the staggered mesh interpolation errors
in these situations.
This may also
...
% (i.e. \ref{eqn:leading-order-1} and \ref{eqn:leading-order-2})
% implies:
...
% mesh interpolation errors
% (i.e. \ref{eqn:leading-order-1} and \ref{eqn:leading-order-2}) are
