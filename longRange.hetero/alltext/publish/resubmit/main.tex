...
xb``interlacing'' technique~\cite{chen1974reduction,eastwood1976optimal}
was applied to SPME
...
technique, which assumes the error  is dominated by  short-range correlation.
This
...
is considered important.  Therefore, in this case, we define the nearest neighbor by the
...
the self-interaction term is low comparing with the SPME
...
\item We explicitly gave the expression of the self-interaction term
  ...
  expression helped removing the self-interaction term at a comparatively low
  computational cost.
