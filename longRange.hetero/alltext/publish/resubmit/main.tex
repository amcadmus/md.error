...
certain neighboring range or charges connected by chemical bonds.
...
charge of $q_{\textrm{O}} = -0.834e$.  \recheck{ There are two types
  of correlation in the system: the bonded correlation and the
  non-bonded correlation. The former originates from the rigid H-O
  bond and H-O-H angle within one molecule. The latter originates from
  the intermolecular non-bonded interaction: the van der Waals and
...
molecule. 
\recheck{
It sould be noted that we define the force error by inserting a
...
system. Therefore, the correlation error
...
Throughout this paper, we also do not calculated this interaction.
% the intramolecular non-bonded
...
\recheck{ If the system is homogeneous and isotropic, the pair
  ...
  Following the same discussion as the bonded correlation,
  the non-bonded correlation error can also be estimated.
  ...
  are the RDFs of oxygen-oxygen, oxgygen-hydrogen and hydrogen-hydrogen,
  respectively. $\rho_{_{\textrm{H}}}$ and $\rho_{_{\textrm{O}}}$ are
  the homogeneous number densities
  of the hydrogen and oxgygen atoms, respectively.
  ...
  and \eqref{eqn:nb-nna-2} are converging. Practically, we integrate
  them to a upper bond denoted by $R_g$.

  It can be seen that both the bonded and non-bonded correlation error
  estimates require some uniformity of the correlation in the system:
  the bonded estimate requires the uniformity of the water
  direction, while the non-bonded estimate requires the uniformity
  of the density distribution and the isotropicity of the system.
  These restrictions should be carefully investigated before using
  these error estimates.
  If the restrictions are seriously perturbed,
  one can either uses these estimates for 
...
\begin{figure}
  ...
  \caption{
    ...
    The red solid line: positive charge of Example 1.
    The red dashed line: positive charge of Example 2.
    The blue line: negative charge.
...
\begin{figure}
  ...
  \caption{
    ...
    randomized COM testing case, calculated by
...
\recheck{ Thanks to the reviewer of this paper, one likely
  ...
  being anti-correlated due to opposing charge signs.
  The extreme case is the overlapping of one oxygen atom with two
  hydrogen atoms, the charges cancels, so the error vanishes.
  In case of the TIP3P water model, the H-O distence is 0.1~\textsf{nm},
  which is much smaller than the typical
  center-of-mass distance between two neighboring water
  molecules (0.31~\textsf{nm}),
  therefore, the bonding correlation tends to neutralize the molecule
...
\recheck{
  Since we
  mainly worry about the validity of the leading order
  approximation made in
  Eqn.~\eqref{eqn:leading-order-1} and \eqref{eqn:leading-order-2},
  we further varify the error estimate
  of the staggered mesh Ewald method 
  by calculating
  the real error of the following testing case
  (we call it the randomized COM testing case):
  randomly redistribute the water molecules according to the same
  center-of-mass (COM) distribution, while keeping
  ...
  Fig.~\ref{fig:tmp-add3} and \ref{fig:tmp-add4}) of its
  actual error with the estimate validates the correctness of 
  the error estimate and 
  the leading order
  approximation~\eqref{eqn:leading-order-1} and \eqref{eqn:leading-order-2}.

  In this example, the non-bonded nearest
  neighbor approximation cannot be used directly in this example, 
  because the system is inhomogeneous, and the RDFs are ill-defined.
  However, the system contains a comparatively homogeneous liquid region,
  where we can define the RDFs and calculate the non-bonded
  nearest neighbor
  approximation. The result error estimate is presented by the
  dashed blue line in Fig.~\ref{fig:water-error}.
  The upper bond $R_g$ for integrating Eqn.~\eqref{eqn:nb-nna-1} and
  \eqref{eqn:nb-nna-2} is chosen to be $1.2$~\textsf{nm}. A good
  converging with respect to the integrating rand
  is observed (for clarity, not shown in the figure),
  when $R_g$ is larger than $0.6$~\textsf{nm}.
  As indicated by the figure, the error estimate is further improved.
  However, there is still a discrepancy between the real and estimate
  error. Currently we cannot explain. It may originate from 
  assuming the testing particle is independent with the system, but
  we cannot check this point, because the correlation of the testing
  particle with the system is beyond the theoretical framework of the
  present paper.
  From Fig.~\ref{fig:water-error}, we conclude that
  the bonded correlation dominate the correlation error, compared to
  the non-bonded correlation. Therefore, if not state otherwise,
...
\begin{figure}
  ...
  \caption{Example 3: the actual and estimated RMS error of the liquid
    ...
    the error estimates with and without nearest neighbor
    ...
    $240\times 120\times 120$, respectively. The order of B-spline
    interpolation is 6.
  }
  \label{fig:tmp-add1}
\end{figure}
...
\begin{figure}
  ...
  \caption{Example 3: the actual and estimated RMS error of the liquid
    ...
    The plotted orders of B-spline interpolation are $n=4$, 6 and 8.
    The reciprocal space grid spacings $h$ is approximately
...
achieves the nearly best accuracy and should be used in simulations.
For all cut-off radii,
the quality of the direct error estimate is satisfactory when the
error is small than $10^{-1}\:\textsf{kJ/(mol nm)}$.
...
the smaller grid spacing and larger interpolation order
enable more accuracy force computation. 
Unlike the uncorrelated testing examples 1 and 2, the actual precision of the 
original ik- and analytical differentiation are nearly the same.
This implies that the latter may be more preferable than
the former for the water simulation,
because it needs only one half FFTs as the former, which could be
highly advantageous for massive parallel simulations.
In most testing cases the staggered mesh ik- differentiation is
equivalent to the staggered mesh analytical differentiation. However,
...
are still close.
With the nearest neighbor approximation, the quality of the
reciprocal error estimates is impressively improved, especially
for the original analytical differentiation and the staggered mesh
Ewald method.
The same as discussed before, we also compare the error of the
randomized COM testing case with the error esitmates. 
In most cases, they are perfectly consistent, which strongly supports
the bonded nearest neighbor approximation proposed by this paper.
However, as expected, when the
leading order approximations to the staggered
mesh interpolation errors
Eqn.~\eqref{eqn:leading-order-1} and \eqref{eqn:leading-order-2} are
not good (see $n=4$ and small $\beta$),
...
\begin{figure}
  ...
  \caption{Example 3: the actual and estimated RMS error of the liquid
    ...
    The blue ``$\boxdot$'' indicates the radomized COM testing case,
    calculated by the
    staggered mesh analytical differentiation. The same results
    of the ik-differentiation are not shown because they are overalpping
    with the analytical differentiation.
