	...
	const double & cellSize_)
{
  ...
  cellSize.x = box.x / double(nCell.x);
  cellSize.y = box.y / double(nCell.y);
  cellSize.z = box.z / double(nCell.z);
  ...
  double rho = double(numAtom) / (box.x * box.y * box.z);
...
void CellList::
rebuild (const std::vector<std::vector<double > > & coord)
{
  clear ();
...
unsigned CellList::
calCellIndex (const std::vector<double > & coord) const
{
		      ...
		      const IntVectorType nNei_)
{
  ...
  std::vector<unsigned >::iterator newend;
  newend = std::unique (cells.begin(), cells.end());
