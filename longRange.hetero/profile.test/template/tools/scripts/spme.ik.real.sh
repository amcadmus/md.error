...
rm -f $errors_dir/parameters.spme.ik.real.sh
cp parameters.sh $errors_dir/parameters.spme.ik.real.sh
...
do
    ...
    tools/real.error/compare.force --config $record_dir/$i --charge-table $record_dir/charge.tab --refh $real_h --force-0 $record_dir/force.ref.$iname.out --force-1 force.spme.ik.out -o error.spme.ik.$iname.out -m meanf.spme.ik.$iname.out &>> $mylog
