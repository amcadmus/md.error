...
const static double global_inte_gr_prec = 1e-6;
...
static void loadRDFFile (const string & fname,
			 ...
			 std::vector<double > & grv)
{
  ...
  double tmp0, tmp1;
  while (fscanf (fp, "%lf%lf", &tmp0, &tmp1) == 2){
    grx.push_back (tmp0);
    grv.push_back (tmp1);
  }
  fclose (fp);
	 ...
	 const string & fileHH)
{
  ...
  double myChargeO = charges[0];
  double myChargeH = charges[1];
  double myRhoO = rhoMol;
  double myRhoH = 2. * myRhoO;
