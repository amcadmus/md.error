...
The calculation of long-range electrostatic interaction is a very
...
the cut-off and neighbor list algoritm does not work, and moreover the
energy of the system is only conditionally convergent.  One of the
first algoritms handling this problem is the Ewald summation that
dates back to 1920's~\cite{ewald1921die}.  However, it is shown that
with a careful tuning of working parameters, the optimal computational
cost of the Ewald summation is of $\mathcal
O(N^{3/2})$~\cite{perram1988asc}, which is not feasible for morden
large scale molecular simulations. The particle mesh Ewald (PME)
...
(P3M) method~\cite{hockney1988computer, deserno1998mue1} are the most
popular Ewald-based fast algorithms that reduces the computational
cost to $\mathcal O(N\log N)$. More recently, the staggered mesh or
``interlacing'' technique firtly developed in
1970's~\cite{chen1974reduction,eastwood1976optimal} is applied to SPME
(called staggered mesh Ewald)~\cite{cerutti2009staggered} and P3M
(called interlaced P3M)~\cite{neelov2010interlaced}, and is showed to
improve the accuracy of these algorithms.

The fast algoritms have achieved great success in the computer
simulation of charged molecular systems over the past three decades.
However, the fact that the user should provide six working parameters
for these methods may be problematic: it is diffcult to try out the
most efficient parameter set, and an arbitary choice may lead to
qualitively wrong simulation result. This problem can be solved by an
in-depth study of the error introduced by these algrotims, namly the
error estimate~\cite{petersen1995accuracy, deserno1998mue2,
...
parameter.  By these error estimates, a work flow was designed to
automatically determine the nearly optimal combination of parameters,
in the sense that it is fastest one given a demanding
...
uncorrelated), but neither of them is satisfactorily fulfilled in most
realistic systems of practical interest.  For example, the lipid biomembrane
solving in water: the carbon chains of the lipids carry no charge, and
the charges are correlated by covalent bonds, van der Waals
interaction, hydrogen bonding network, etc..  Therefore, the current
...
The purpose of this research is to develop an realiable force error
estimate in inhomogeneous and strongly correlated molecular systems.
We estimate the force error because the application is the molecular
dynamics (MD) simulation.  Firstly, a most general error estimate
frame work for the pairwise interactions is set up. We prove that the
error is decomposed into three additive parts: the homogeneity error,
the inhomogeneity error and the correlation error~\cite{wang2012}. The
...
in an inhomogeneous water system.  Following this framework, we
develop the error estimate for the Ewald summation, the SPME method
and the staggered method Ewald method. We study the SPME method
because it is the one of the most popular long-range algorithms among
the mainstream MD simulation packages, for example,
AMBER~\cite{case2005amber}, GROMACS~\cite{van2005gromacs,
  hess2008gromacs} and NAMD~\cite{phillips2005scalable}.  The estimate
...
\begin{corollary}\label{thm:tmp2}
  ...
  \end{align}
  then the error estimates are in the convolution form, namely:
...
\item
  ...
  By writting the error estimates in the covolution form in
...
In this section, we consider the nearest neighbor approximation of the
correlation error. Here the definition of the ``nearest neighbor'' is
very flexible, basically it means the neigbors of a charge that are
not very far away. It could be defined by the charges falling in some
certain neighboring range or charge connected by a chemical bonds.
...
%   \bigg\rangle.
% \end{align}
We seperately consider the  contribution from nearest neigbors
...
\rho_q(\v r')\rho_q(\v r'')$, where $\#(\Omega_j)$ is the number of nearest neighbors considered. This approximation is valid only when the number
of neigbors considered is small comparing with the number of charges
in the system.
...
impove the accuracy by considering more and more neigbors as long as
the number of neigbors is small comparing with the size of the system.
...
To calculate the nearest neighbor approximated correlation error,
we assume the error force kernel
has the form $\v K(\v r_i , \v r_j) = \v K(\v r_i - \v r_j)$, so
the integrals can be written in the covolution form and accelerated by the FFT.
...
\end{align}
The error force kernel has the form of Eqn.~\eqref{eqn:cor-kernel},
...
third law is preserved by the staggered mesh analytical differetiation.
...
\begin{figure}
  ...
  \caption{
    ...
    ``$\times$'': the real error.
    Dashed line: the RMS error estimate.
    Solid  line: the RMS error estimate, with the nearest neighbor
    approximation of correlation error.
    The working parameters are the same as exaple 1.
    The black arrows indecate the improvement
...
\begin{figure}
  ...
  \caption{
    ...
    Blue solid line: the error estimate including the nearest
    neighbor approximation of the correlation error.
    The RMS errors are averged over $y$ and $z$ directions, and are
    plotted against $x$ axis.
    The working parameters are the same as example 1.
    The black arrow indecates the improvement
    of error estimate by including nearest neighbor
    approximation of correlation error.
  }   
...
and Fig.~\ref{fig:water-error}.
The staggered method ik- and analytical differentiation
are also equivalent in this case, so they are mentioned by the ``staggered
mesh Ewald''  for convenience.
In this case, due to the correlation of the charge positions, the assumption
of independent positions is disturbed, so all estimates deviate from
the real errors.
...
and the staggered mesh Ewald method are
overestimated by 2.6, 4.7 and 6.8 times, respectively.
Notice the only difference between this example and example 1 is the correlation
of the charges in the system,
so it plays a very important role in the RMS error. 
...
correlation error, the quality of all error estimates is imporved greatly
...
and staggered mesh Ewald method
...
assume the homogeneity and uncorrelation of the system,
which is a too strong assumption for most real molecular simulations.
...
The error estimates for the reciprocal space fast algoritms
...
The third and fourth terms are definitly high-wave-number terms.
