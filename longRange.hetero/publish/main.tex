...
\begin{abstract}
  ...
  some state-of-art algorithms of long-range electrostatic interaction
  in inhomogeneous and correlated molecular systems.
  They are the Ewald summation, the smooth particle mesh Ewald (SPME) and
  the staggered mesh Ewald method. Two branches of force computation, namely
...
\begin{align}
  ...
  {\sum\limits_{l=-\infty}^{+\infty}
    \big(\dfrac{2\pi m_\alpha}{K_\alpha} + 2\pi l\big)^{-n}}.
\end{align}
Due to symmetricity of $\v g(\v m)$ and $A(\v m, \v r)$, the
...
In this paper, we proposed the error estimates for some state-of-art
...
the staggered mesh Ewald (both ik- and analytical differentiation).
...
were greatly improved by including the first neighbor approximation
