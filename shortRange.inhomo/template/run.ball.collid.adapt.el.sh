...
function set_parameter () {
    ...
    sed -e "/double prec/s/=.*/= $collision_prec;/g"|\
...
set_parameter tools.collision/simulation.hetero.cutoff.elcufft/app/lj.nve.cu
make -j4 -C tools.collision/simulation.hetero.cutoff.elcufft/ >& make.log
...
cd ..
workdir=`pwd`
command="$workdir/tools.collision/simulation.hetero.cutoff.elcufft/lj.nve conf.gro $collision_nstep $device"
