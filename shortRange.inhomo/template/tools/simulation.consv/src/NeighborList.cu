				  ...
				  mdError_t *			ptr_de)
{
  ...
  if (ii < numberAtom){
    ref = coord[ii];
    reftype = type[ii];
    refrlist = ref.w + nlist.rlistExten;
  ...
  for (IndexType targetBlockId = 0;
	...
	ScalorType rlist = target[kk].w + nlist.rlistExten;
	// ScalorType rlist = rcut[kk + targetBlockId * blockDim.x] + nlist.rlistExten;
	if (rlist < refrlist) rlist = refrlist;
