...
set_parameter tools/simulation.consv/app/lj.nhc-xtc.cu
make -j4 -C tools/simulation.consv/ &> make.log
rm -f conf.gro
cp $confFile conf.gro
workdir=`pwd`
command="$workdir/tools/simulation.consv/lj.nhc-xtc conf.gro $nstep $device"
