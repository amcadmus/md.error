				       ...
				       DeviceMDData * ddata)
{
  ...
  cudaMemcpy (ddata->fcx, hdata->fcx, sizef, cudaMemcpyDeviceToDevice);
  cudaMemcpy (ddata->fcy, hdata->fcy, sizef, cudaMemcpyDeviceToDevice);
  cudaMemcpy (ddata->fcz, hdata->fcz, sizef, cudaMemcpyDeviceToDevice);
				...
				DeviceMDData * ddata)
{
  ...
  cudaMalloc ((void**) &ddata->fcx, sizef);
  cudaMalloc ((void**) &ddata->fcy, sizef);
  cudaMalloc ((void**) &ddata->fcz, sizef);
			...
			const IndexType indx2)
{
  ...
  ddata2->fcx[indx2] = ddata1->fcx[indx1];
  ddata2->fcy[indx2] = ddata1->fcy[indx1];
  ddata2->fcz[indx2] = ddata1->fcz[indx1];
