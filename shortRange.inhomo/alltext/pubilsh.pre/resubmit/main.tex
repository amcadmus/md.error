...
{ One possible way to avoid the cut-off difficulty is to
  ...
  % Shifting the direct part of the Ewald summation actually
  % violates the Coulomb's law in the way that depends on the
  % splitting parameter, so it should be treated carefully.
  % Here we would like to suggest treating the shifting approach
  % as a special case 
}
...
% standard Lennard-Jones particles is set up in a tube with periodic
...
\begin{figure}
  ...
  \caption{(Color online)
    ...
    The cut-off of
    the adaptive cut-off method is $r_c^\ast$ means its
    control error is the maximum error of a uniform cut-off simulation
...
radius. {
  ...
  % the extend of which is shown by the
  % the amount of the correction calculated by the pressure correction method.
}  
% However, even at $r_c^\ast=10$, this correction is not
...
{ The precise calculation of the correction force needs the
  ...
  % In
  % the latter case, the MD simulation are not trivial anyway, for
  % example, the simulation of critical point.
  % For these two cases, the traditional MD simulation is also not easy
  % to handel.
  In our
  ...
  % or the state of the system changes too fast
  % comparing with the correction force updating fequency
...
It should be noticed that the porposed error estimate and correction
