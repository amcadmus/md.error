...
interactions. This results in a computational cost scaling $\mathit{O}(N^2)$ per time step, which becomes rapidly inefficient as the number
...
$\mathit{O}(N)$.  The cut-off radius neglects all pairwise interactions
...
% The spacial inhomogeneity of the system in terms of particle density,
...
a computational cost of $\mathit{O}(N^2)$. To estimate the force error
on the fly, the computational cost of the estimate should be reduced to
at least $\mathit{O}(N\log N)$.  This is achieved by  using the fast Fourier transform. We
...
cost of each force error estimate is $\mathit{O}(N \log N)$,
so the total computational expense of 
this method is at least $\mathit{O}(N^2\log N)$, which is
...
E_{\textrm{C}}$. The computational cost of this algorithm is $\mathit{O}(MN\log N)$.
The idea of this algorithm is based on the observation
...
Since the density profile of the system changes
very slow comparing with the typical time step used in the MD simulation,
the cut-off distribution is not calculated every time step.
Instead,
for example, in the liquid-vapor equilibrium simulation
(see Sec.~\ref{sec:tmp2.1}),
the cut-off distribution is calculated every 20,000 time steps,
so the extra load of adapting the cut-off is only marginal
in the simulation, i.e. proportional to $ \frac{M}{20,000}N\log N$.
...
calculate the energy or force correction. The advantage of our force
...
densities, and the surface tension. The system studied in the present
...
\begin{figure}
  \centering
  \includegraphics[]{pre.error.uniform.eps}
  \caption{(Color online)
    The RMS force error distribution of the 16000 Lennard-Jones
    particle system at $T^\ast=0.85$ with a uniform cut-off radius of
    $r_c^\ast = 7.5$. The green (light gray) squares present the real error. The
    solid blue (dark gray) line is the error estimate by \eqref{eqn:tmp5}. The
    dashed blue (dark gray) line
    is the homogeneity error contribution, while the
    dotted blue (dark gray) line
    is the inhomogeneity error contribution. The
    density distribution of the system is presented by red (middle gray)
    solid line
...
\begin{figure}
  \centering
  \includegraphics[]{pre.rcut.and.error.eps}
  \caption{ (Color online)
    The RMS force error distribution of the 16000
    ...
    presented by the red (middle gray) line. All other notations are the same as
    Fig.~\ref{fig:tmp2}.}
  \label{fig:tmp3}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[]{pre.fcorr.and.error.eps}
  \caption{ (Color online)
    The RMS force error distribution of the 16000
    ...
    force is presented by the solid red (middle gray) line.
    The green (light gray) squares
    present the real error. The solid blue (dark gray)
    line presents the estimated
    homogeneity error. }
  \label{fig:tmp4}
\end{figure}
...
\begin{figure}
  ...
  \caption{(Color online)
    $T^\ast = 0.85$. The convergence of the equilibrium gas and
    ...
    namely the uniform cut-off (URC) in red (middle gray), the
    adaptive cut-off (ARC) in green (light gray)
    and the force correction (LFC) method in blue (dark gray).
    ``The cut-off of
    the adaptive cut-off method is $r_c^\ast$'' means that the
    control error  $\mathcal E^\ast_{\textrm{C}}$
    ...
    $r_c^\ast$ (no more than $r_c^{\textrm{{step}}}$).
  }
  \label{fig:tmp5}
\end{figure}
...
\begin{figure}
  ...
  \caption{(Color online)
    $T^\ast = 0.85$. The convergence of the surface tension
    with respect to the cut-off radius.  This figure presents the
    results of three methods,
    namely the uniform cut-off (URC) in red (middle gray),
    the adaptive cut-off (ARC) in green (light gray)
    and the long-range force correction (LFC) method in blue (dark gray).
    The results with and without long-range pressure
    correction (LPC) are shown together for comparison. The reference
    surface tension, shown by the solid black line, is measured by the
    ...
    or sometimes marginally larger  than
    $r_c^\ast$ (no more than $r_c^{\textrm{{step}}}$).
    % This setting leads to a maximum 
    % cut-off radius that is almost the same as $r_c^\ast$.
  }
  \label{fig:tmp6}
\end{figure}
...
larger homogeneous force error in the bulk liquid or gas regions will not
...
\begin{figure}
  ...
  \caption{
    (Color online)
    The initial setup of cluster $A$ and $B$. The initial
...
\begin{figure}
  \centering
  \includegraphics[]{pre.trajs.eps}
  \caption{(Color online)
    The center of mass trajectory of cluster A. Uniform cut-off
    (URC) $r_c^\ast = 2.5,\ 5.0,\ 7.5$, adaptive cut-off (ARC) $r_c^\ast =
...
\begin{figure}
  ...
  \caption{(Color online)
    The COM distance between cluster $A$ and cluster $B$ versus
    time. Uniform cut-off (URC) $r_c^\ast = 2.5,\ 5.0,\ 7.5$, adaptive
    cut-off (ARC) $r_c^\ast = 5.0$ and long-range force correction (LFC)
...
\begin{figure}
  ...
  \caption{(Color online)
    The colliding clusters (the first row), the RMS force error
...
adaptive cut-off method is updated every 200 time steps.
The third row
...
\begin{figure}
  \centering
  \includegraphics[]{pre.wy.eps}
  \caption{(Color online)
    The angular velocity of cluster $A$ versus time. Uniform
    cut-off (URC) $r_c^\ast = 2.5,\ 5.0,\ 7.5$, adaptive cut-off (ARC)
    $r_c^\ast = 5.0$ and long-range force correction (LFC) $r^\ast_c = 2.5$
    are investigated.}
  \label{fig:tmp11}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[]{pre.ly.eps}
  \caption{(Color online)
    The angular moment of cluster $A$ versus time. Uniform
    cut-off (URC) $r_c^\ast = 2.5,\ 5.0,\ 7.5$, adaptive cut-off (ARC)
...
\begin{figure}
  \centering
  \includegraphics[]{pre.cost.eps}
  \caption{(Color online)
    The computational cost versus time. Uniform
    cut-off (URC) $r_c^\ast = 2.5,\ 5.0,\ 7.5$, adaptive cut-off (ARC)
    $r_c^\ast = 5.0$ and long-range force correction (LFC) $r^\ast_c = 2.5$
    are investigated.
    The computational cost is counted by
    the number of calculated pairwise interactions.
  }
  \label{fig:tmp13}
\end{figure}
...
It should be noticed that the proposed error estimate and correction
...
{
  In the systems tested by the present paper, the computational
  load of the error estimate is small compared to that of the
  short-range interaction.
  However, it should be noticed that
  the cost of the error estimate scales as $\mathit{O}(N\log N)$.
  If the system only has short-range interaction that
  scales as $\mathit{O}(N)$, the load of the error estimate
  ...
  that is usually treated by the $\mathit{O}(N\log N)$ scaling
  Ewald type algorithms~\cite{darden1993pme, essmann1995spm, deserno1998mue1},
