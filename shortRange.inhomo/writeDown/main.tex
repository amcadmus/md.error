...
F_{\textrm{corr}}(\v r)] = \textrm{Var}[\Delta\v F(\v r)]$, which
contains no heterogeneous contribution.
...
profile is assumed to be heterogeneous on only one direction, and the
...
the maximum RMS force error and the computational expense of the the
...
bulk regions dominate the system, and the heterogeneity produces large
...
uniform/adaptive cut-off methods converge at $r_c^\ast = 7.5$.  The
...
3.5$ and uniform cut-off radius method at $r_c^\ast = 7.5$, the force
accuracy of the former method is 3 times worse than the later (see
Tab. \ref{tab:tmp1}). The equilibrium density accuracy is better than
...
cancelled by itself in a long time average.  The heterogeneity error,
